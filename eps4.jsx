...
(function() {
    try {
        ...
        alert("Image 1 scaled to: " + ptToMm(placed1.width).toFixed(1) + 
              "mm x " + ptToMm(placed1.height).toFixed(1) + "mm\n" +
              "Scale factor: " + scale1.toFixed(1) + "%\n" +
              "Position: " + ptToMm(placed1.left).toFixed(1) + "mm, " + 
              ptToMm(placed1.top).toFixed(1) + "mm");
        
        // Force redraw to ensure bounds are updated
        app.redraw();
        ...
        // Position second image to the right of first image
        var image2Left = placed1.left + placed1.width + separation;
        placed2.left = image2Left;
        placed2.top = placed1.top; // Same top position
        
        alert("Image 2 positioned at: " + ptToMm(placed2.left).toFixed(1) + 
              "mm, " + ptToMm(placed2.top).toFixed(1) + "mm\n" +
              "Gap from Image 1: " + ptToMm(separation).toFixed(1) + "mm");
        ...
        infoText.contents = "Total width: " + ptToMm(totalWidth).toFixed(1) + "mm | " +
                           "Each image: " + ptToMm(placed1.width).toFixed(1) + "x" + 
                           ptToMm(placed1.height).toFixed(1) + "mm | Gap: " +
                           ptToMm(separation).toFixed(1) + "mm";
        ...
        alert("🎉 SUCCESS!\n\n" +
              ...
              "📏 Each: " + ptToMm(placed1.width).toFixed(1) + " x " + 
              ptToMm(placed1.height).toFixed(1) + "mm\n" +
              "📐 Total width: " + ptToMm(totalWidth).toFixed(1) + "mm\n" +
              "🔄 Gap: " + ptToMm(separation).toFixed(1) + "mm");
